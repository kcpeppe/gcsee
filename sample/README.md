# GCSee introductory sample

This sample shows how to use [GCSee](../README.md) to analyze a GC log file and report on total heap occupancy after a GC cycle has been completed. For more details on how GCSee and this sample work, visit [Introducing Microsoft GCSee](https://devblogs.microsoft.com/java/introducing-microsoft-gctoolkit/).
...
java --module-path target/classes:target/lib --module com.kodewerk.gcsee.sample/com.kodewerk.gcsee.sample.Main ../gclogs/preunified/cms/defnew/details/defnew.log
```
...
1. There is no `gclogs` directory in the _top-level_ directory. The sample uses `unit-test` data from building GCSee. Either run `mvnw test` from the _top-level_ directory, or use `-DgcLogFile=<path-to-gc-log-file>` argument to specify a log file.
